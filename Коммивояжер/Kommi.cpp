...
void Kommi::init() {
	m_names = {};
	m_matrix = {};
	cout << "Количество вершин: "; cin >> m_count;

	for (int i = 0; i < m_count; i++) {
		string tmp;
		cout << "Название вершины " << (i + 1) << ": "; cin >> tmp;
	...
	for (int i = 0; i < m_count; i++)
	{
		...
		for (int j = 0; j < m_count; j++)
		{
			...
			{
				int in;
				cout << "Длина маршрута из вершины " << m_names[i] << " в вершину " << m_names[j] << ": "; cin >> in;
				t.push_back(in);
			}
		}
...
KTree* Kommi::Solve(bool onlyInclude = false) {
	...
	while (parent->solution.size() != m_count) {
		wayLen = parent->value; // длина пути
